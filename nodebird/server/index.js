...
const { KAKAO_ID, KAKAO_SECRET, SERVER_URL, DB_HOST, DB_USER, DB_PASSWORD, DB_DATABASE, DB_PORT } = process.env;
  ...
  new KakaoStrategy(
    {
      ...
      callbackURL: `${SERVER_URL}/auth/kakao/callback`,
    },
    (accessToken, refreshToken, profile, done) => {
      ...
      const userid = profile.id.toString().substring(0, 6); // Extract the first 6 digits of response.id

      done(null, { userid, username, email, usertype });
    }
  )
);
...
app.post("/kakao-login", (req, res) => {
  ...
  // Define default values for address, detailedaddress, and phonenumber
  const address = "my_home";
  const detailedaddress = ""; // You can fill this if you have a default detailed address
  ...
  connection.query(sql, [userid], (err, results) => {
    ...
    if (results.length === 0) {
      ...
      const insertSql = "INSERT INTO user (userid, username, email, password, usertype, address, detailedaddress, phonenumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
      connection.query(insertSql, [newUser.userid, newUser.username, newUser.email, newUser.password, newUser.usertype, newUser.address, newUser.detailedaddress, newUser.phonenumber], (err, results) => {
        if (err) {
          return res.status(500).json({ success: false, message: "서버 에러 발생" });
        }
        return res.status(200).json({ success: true, message: "사용자 생성 완료" });
      });
    } else {
      // If the user already exists, return success
      return res.status(200).json({ success: true, message: "이미 등록된 사용자입니다." });
    }
  });
});
