      ...
      <div className="ordersheet_contanier">
        <div className="order_detail_box">
          ...
          <div className="order_product_box">
            ...
            <hr></hr>총 주문 상품 금액 : {totalProductAmount().toLocaleString()} 원<hr></hr>
          </div>
          <div
            className={
              viewOrdererSheet
        ...
        <div className="ordersheet_summary_container">
          ...
          {userCart.length ? (
            <h2>
              상품명 : <span className="emphasis_positive">{userCart[0].name}</span>{" "}
              {userCart.length > 1 && (
                <span>
                  외 <span className="emphasis_positive">{userCart.length - 1}</span> 건
                </span>
              )}
            </h2>
          ) : (
